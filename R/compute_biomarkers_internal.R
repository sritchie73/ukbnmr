...
extended_ratios_compute <- function(x) {
  ...
  tryAssign(x[, XXL_VLDL_FC_pct_C := XXL_VLDL_FC / XXL_VLDL_C * 100])
  tryAssign(x[, XL_VLDL_FC_pct_C := XL_VLDL_FC / XL_VLDL_C * 100])
  tryAssign(x[, L_VLDL_FC_pct_C := L_VLDL_FC / L_VLDL_C * 100])
  tryAssign(x[, M_VLDL_FC_pct_C := M_VLDL_FC / M_VLDL_C * 100])
  tryAssign(x[, S_VLDL_FC_pct_C := S_VLDL_FC / S_VLDL_C * 100])
  tryAssign(x[, XS_VLDL_FC_pct_C := XS_VLDL_FC / XS_VLDL_C * 100])
  tryAssign(x[, L_LDL_FC_pct_C := L_LDL_FC / L_LDL_C * 100])
  tryAssign(x[, M_LDL_FC_pct_C := M_LDL_FC / M_LDL_C * 100])
  tryAssign(x[, S_LDL_FC_pct_C := S_LDL_FC / S_LDL_C * 100])
  tryAssign(x[, IDL_FC_pct_C := IDL_FC / IDL_C * 100])
  tryAssign(x[, XL_HDL_FC_pct_C := XL_HDL_FC / XL_HDL_C * 100])
  tryAssign(x[, L_HDL_FC_pct_C := L_HDL_FC / L_HDL_C * 100])
  tryAssign(x[, M_HDL_FC_pct_C := M_HDL_FC / M_HDL_C * 100])
  tryAssign(x[, S_HDL_FC_pct_C := S_HDL_FC / S_HDL_C * 100])

  tryAssign(x[, XXL_VLDL_CE_pct_C := XXL_VLDL_CE / XXL_VLDL_C * 100])
  tryAssign(x[, XL_VLDL_CE_pct_C := XL_VLDL_CE / XL_VLDL_C * 100])
  tryAssign(x[, L_VLDL_CE_pct_C := L_VLDL_CE / L_VLDL_C * 100])
  tryAssign(x[, M_VLDL_CE_pct_C := M_VLDL_CE / M_VLDL_C * 100])
  tryAssign(x[, S_VLDL_CE_pct_C := S_VLDL_CE / S_VLDL_C * 100])
  tryAssign(x[, XS_VLDL_CE_pct_C := XS_VLDL_CE / XS_VLDL_C * 100])
  tryAssign(x[, L_LDL_CE_pct_C := L_LDL_CE / L_LDL_C * 100])
  tryAssign(x[, M_LDL_CE_pct_C := M_LDL_CE / M_LDL_C * 100])
  tryAssign(x[, S_LDL_CE_pct_C := S_LDL_CE / S_LDL_C * 100])
  tryAssign(x[, IDL_CE_pct_C := IDL_CE / IDL_C * 100])
  tryAssign(x[, XL_HDL_CE_pct_C := XL_HDL_CE / XL_HDL_C * 100])
  tryAssign(x[, L_HDL_CE_pct_C := L_HDL_CE / L_HDL_C * 100])
  tryAssign(x[, M_HDL_CE_pct_C := M_HDL_CE / M_HDL_C * 100])
  tryAssign(x[, S_HDL_CE_pct_C := S_HDL_CE / S_HDL_C * 100])

  # Compute ratio of free cholesterol to esterified cholesterol
  tryAssign(x[, XXL_VLDL_FC_by_CE := XXL_VLDL_FC / XXL_VLDL_CE])
  tryAssign(x[, XL_VLDL_FC_by_CE := XL_VLDL_FC / XL_VLDL_CE])
  tryAssign(x[, L_VLDL_FC_by_CE := L_VLDL_FC / L_VLDL_CE])
  tryAssign(x[, M_VLDL_FC_by_CE := M_VLDL_FC / M_VLDL_CE])
  tryAssign(x[, S_VLDL_FC_by_CE := S_VLDL_FC / S_VLDL_CE])
  tryAssign(x[, XS_VLDL_FC_by_CE := XS_VLDL_FC / XS_VLDL_CE])
  tryAssign(x[, L_LDL_FC_by_CE := L_LDL_FC / L_LDL_CE])
  tryAssign(x[, M_LDL_FC_by_CE := M_LDL_FC / M_LDL_CE])
  tryAssign(x[, S_LDL_FC_by_CE := S_LDL_FC / S_LDL_CE])
  tryAssign(x[, IDL_FC_by_CE := IDL_FC / IDL_CE])
  tryAssign(x[, XL_HDL_FC_by_CE := XL_HDL_FC / XL_HDL_CE])
  tryAssign(x[, L_HDL_FC_by_CE := L_HDL_FC / L_HDL_CE])
  tryAssign(x[, M_HDL_FC_by_CE := M_HDL_FC / M_HDL_CE])
  tryAssign(x[, S_HDL_FC_by_CE := S_HDL_FC / S_HDL_CE])

  # Compute lipid percentages in lipoprotein classes
  tryAssign(x[, VLDL_CE_pct := VLDL_CE / VLDL_L * 100])
  tryAssign(x[, VLDL_FC_pct := VLDL_FC / VLDL_L * 100])
  tryAssign(x[, VLDL_C_pct := VLDL_C / VLDL_L * 100])
  tryAssign(x[, VLDL_PL_pct := VLDL_PL / VLDL_L * 100])
  tryAssign(x[, VLDL_TG_pct := VLDL_TG / VLDL_L * 100])

  tryAssign(x[, LDL_CE_pct := LDL_CE / LDL_L * 100])
  tryAssign(x[, LDL_FC_pct := LDL_FC / LDL_L * 100])
  tryAssign(x[, LDL_C_pct := LDL_C / LDL_L * 100])
  tryAssign(x[, LDL_PL_pct := LDL_PL / LDL_L * 100])
  tryAssign(x[, LDL_TG_pct := LDL_TG / LDL_L * 100])

  tryAssign(x[, HDL_CE_pct := HDL_CE / HDL_L * 100])
  tryAssign(x[, HDL_FC_pct := HDL_FC / HDL_L * 100])
  tryAssign(x[, HDL_C_pct := HDL_C / HDL_L * 100])
  tryAssign(x[, HDL_PL_pct := HDL_PL / HDL_L * 100])
  tryAssign(x[, HDL_TG_pct := HDL_TG / HDL_L * 100])

  # Compute cholesterol percentages
  tryAssign(x[, VLDL_FC_pct_C := VLDL_FC / VLDL_C * 100])
  tryAssign(x[, LDL_FC_pct_C := LDL_FC / LDL_C * 100])
  tryAssign(x[, HDL_FC_pct_C := HDL_FC / HDL_C * 100])

  tryAssign(x[, VLDL_CE_pct_C := VLDL_CE / VLDL_C * 100])
  tryAssign(x[, LDL_CE_pct_C := LDL_CE / LDL_C * 100])
  tryAssign(x[, HDL_CE_pct_C := HDL_CE / HDL_C * 100])

  # Ratios of free cholesterol to esterified cholesterol:
  tryAssign(x[, VLDL_FC_by_CE := VLDL_FC / VLDL_CE])
  tryAssign(x[, LDL_FC_by_CE := LDL_FC / LDL_CE])
  tryAssign(x[, HDL_FC_by_CE := HDL_FC / HDL_CE])

  # Lipid and cholesterol fractions in total serum
  tryAssign(x[, Total_CE_pct := Total_CE / Total_L * 100])
  tryAssign(x[, Total_FC_pct := Total_FC / Total_L * 100])
  tryAssign(x[, Total_C_pct := Total_C / Total_L * 100])
  tryAssign(x[, Total_PL_pct := Total_PL / Total_L * 100])
  tryAssign(x[, Total_TG_pct := Total_TG / Total_L * 100])

  tryAssign(x[, Total_FC_pct_C := Total_FC / Total_C * 100])
  tryAssign(x[, Total_CE_pct_C := Total_CE / Total_C * 100])
  tryAssign(x[, Total_FC_by_CE := Total_FC / Total_CE])

  # Omega 3 and Omega 6 percent of PUFA
  tryAssign(x[, Omega_3_pct_PUFA := Omega_3 / PUFA * 100])
  tryAssign(x[, Omega_6_pct_PUFA := Omega_6 / PUFA * 100])
